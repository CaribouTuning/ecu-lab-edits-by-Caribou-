...
export function simulateDragRun({
  ...
  const shiftSec = arc?.ffsEnabled && box.box === 'manual' ? box.shiftSec * 0.55 : box.shiftSec;
  ...
  const TC_TAU_S = { spark: 0.02, fuel: 0.01, throttle: 0.12, boost: 0.5 };
  ...
  while (x < QUARTER_MILE_M && t < DRAG_TIMEOUT_S) {
    ...
    if (tq) {
      // The calibration's torque ceiling for this gear, and traction control's cut.
      engineNm = Math.min(engineNm, read1(tq.limitByGear, gearIdx + 1));
      const deliveredCut = tq.tcMethod === 'fuel' ? Math.ceil(tcAct * cylCount - 1e-9) / cylCount : tcAct;
      engineNm *= 1 - clamp(deliveredCut, 0, 0.95);
    }
    ...
    if (tc) {
      ...
      tcCmd = clamp(tcCmd + (excess > 0 ? excess * tq.tcGain * 0.01 : -0.6) * dt * 10, 0, 0.95);
