...
export function evaluatePoint({
  ...
  if (ign && deadFrac > 0) {
    ...
    egtC = egtC * (1 - airOnly) + (chargeK - KELVIN_OFFSET + 150) * airOnly
      + unlitHot * ECU_AFTERBURN_RISE_C;
  }
 ...
 */
export const ECU_AFTERBURN_RISE_C = 260;
...
function ecuKnockRetard(margin, K) {
  ...
  // Once it hears knock the controller retards in steps and creeps back until it hears
  // it again, so it settles where knock is just audible — overshooting that edge by up to
  // half a step on the safe side. A deadband narrower than that half step is covered; a
  // wider one leaves the engine running the difference into knock.
  const deficit = Math.max(0, -margin);
  const deadband = K.deadbandDeg ?? 0;
  const uncovered = Math.max(0, deadband - (K.stepDeg ?? 2) / 2);
  const heard = deficit > deadband ? deficit - uncovered : 0;
...
function ignitionState({ cyc, usedTiming, lambda, E }) {
  ...
  const pSparkBar = (cyc.trappedPa * Math.pow(vIvc / Math.max(vSpark, 1e-9), 1.3)) / 1e5;
  ...
  const kvNeeded = 1.5 + 1.5 * gap * Math.pow(Math.max(pSparkBar, 0.5), 0.85);
  const kvAvailable = E.spark?.kvAvailable ?? 45;
  // Breakdown is statistical: within a couple of kV of the coil's ceiling some events
  // make it across the gap and some do not.
  const sparkMiss = clamp((kvNeeded - kvAvailable) / 4 + 0.5, 0, 1);
  ...
  const leanLimit = 1.6 - 1.2 * r;
  const richLimit = 0.45 + 0.25 * r;
  const mixMiss = lambda > leanLimit ? clamp((lambda - leanLimit) / 0.25, 0, 1)
    : lambda < richLimit ? clamp((richLimit - lambda) / 0.15, 0, 1) : 0;
  return {
