...
import { knockDetection, knockNoiseV, knockSignalPerDegV } from './knockSensor.js';
...
const PLENUM_TO_DISPLACEMENT = 1.4;
...
function filmParams(ectC) {
  const cold = clamp((80 - ectC) / 100, 0, 1);
  return { X: 0.12 + 0.4 * cold, tau: 0.12 + 0.7 * cold };
}
...
export function liveStepEcu(st, dt, input, cfg) {
  ...
  if (s.cranking && s.rpm > RUNNING_RPM) { s.cranking = false; s.running = true; }
  ...
  const demandA = 22 + (aux.lights ? 45 : 0) + (aux.ac ? 16 : 0);
  let altNm = 0;
  if (s.cranking) e.volts = 10.2;
  else if (s.running) {
    const capA = 120 * clamp((s.rpm - 500) / 1800, 0.12, 1);
    const suppliedA = Math.min(demandA, capA);
    e.volts = demandA <= capA ? 14.2 - 0.004 * demandA : 12.5 - (demandA - capA) * 0.015;
    altNm = (e.volts * suppliedA) / (Math.max(50, s.rpm) * 2 * Math.PI / 60) / 0.55;
  } else e.volts = 12.6;
  ...
  if (s.running && idleZone && pedal < 3) {
    ...
    if (holding) { /* hold base air through the flare */ } else if (s.rpm < 2000) {
      ...
      if (err > cal.idle.antiStallRpm) s.idleTrim = clamp(s.idleTrim + cal.idle.antiStallAir * (dt / 0.05) * 0.25, 1, 34);
    } else {
      ...
      s.idleTrim += (3 - s.idleTrim) * cal.idle.bleed * (dt / 0.05);
    }
  }
  ...
  if (s.limiterCut) cutFrac = 1;
    ...
    if (cal.limiter.mode === 'retard') limiterRetard = 15 * soft;
    else cutFrac = soft;
  }
  ...
  if ((s.running || s.cranking) && s.rpm > 100) {
    ...
    const loadSteady = s.cranking
      ? BARO_KPA * clamp(0.92 - (s.rpm / 1000) * 0.3, 0.6, 0.95)
      : BARO_KPA * clamp(
        0.18 + overlap * 0.0042 + 0.82 * Math.pow(aFrac, 0.75) - 0.28 * nFrac * Math.pow(1 - aFrac, 2),
        0.12, 1,
      );
    ...
    const fillTau = PLENUM_TO_DISPLACEMENT * 120 / Math.max(300, s.rpm);
    ...
    if (turboOn) {
      ...
      target = Math.max(0, bt.target - e.boostCutPsi - (limp ? 99 : 0));
      steps = bt.steps;
    }
    ...
    if (cal.boost.mode === 'closed' && turboOn) {
      ...
      if (Math.abs(err) < cal.boost.iWindowPsi) e.boostI = clamp(e.boostI + cal.boost.ki * err * dt, -60, 60);
    ...
    if (e.overboostT > cal.boost.overboostDelayS) {
      ...
      else e.boostCutPsi = 99;
    }
    if (pedal < 3) { e.fuelCutProt = e.fuelCutProt && e.overboostT > 0; if (e.boostCutPsi >= 99 && e.overboostT === 0) e.boostCutPsi = 0; }
    ...
    const idleSpark = s.running && idleZone && pedal < 3 && s.rpm < 1600
    ...
    if (e.highDet && e.knockRetard === 0) { e.highDetQuiet = (e.highDetQuiet ?? 0) + dt; if (e.highDetQuiet > 10) e.highDet = false; }
    const res = resolveEcuPoint({
      ...
      dyn: {
        ...
        idleSparkDeg: idleSpark, cranking: s.cranking, decel: pedal < 3 && s.rpm > 1600 && !dfco,
        afrTarget: e.afrTargetF, oilC: s.oilC, highDet: e.highDet, launch: launchArmed,
      },
    });
    pt = evaluatePoint(/** @type {any} */ (res.input));
    e.egtK += (pt.egt + 273.15 - e.egtK) * clamp(dt / 0.3, 0, 1);
    ...
    const det = knockDetection({ rpm: s.rpm, peakBar: pt.peakPressure, thresholdV: read1(cal.ignition.knockThreshold, s.rpm), derived, maxRetardDeg: cal.ignition.knockMaxRetard });
    const heard = cal.ignition.knockEnabled && s.running && (knockV > read1(cal.ignition.knockThreshold, s.rpm) || det.falseRetardDeg > 0.5 && Math.random() < det.falseRetardDeg / cal.ignition.knockMaxRetard);
    if (heard) {
    ...
    if (actualKnockDeg > 0) s.knockCount += actualKnockDeg * dt * 8;
    e.knockV = knockV;
    ...
    if (P.leanEnabled && e.sLambda > P.leanLambda && e.sMap > P.leanMinKpa && s.running && cutFrac === 0 && e.cutQuiet > 0.4) e.leanT += dt; else e.leanT = 0;
    if (e.leanT > 0.3) {
      prot.push('lean');
      if (P.leanAction === 'fuel-cut') e.fuelCutProt = true;
      else if (P.leanAction === 'torque') e.torqueRetard = Math.min(20, e.torqueRetard + 20 * dt);
      else e.boostCutPsi = Math.max(e.boostCutPsi, P.boostCutPsi);
    }
    if (P.egtEnabled && pt.egt > P.egtLimitC) { prot.push('egt'); e.extraFuelPct = Math.min(P.egtEnrichMaxPct, e.extraFuelPct + 10 * dt); } else e.extraFuelPct = Math.max(0, e.extraFuelPct - 5 * dt);
    ...
    if (e.oilT > 0.5) { prot.push('oil pressure'); e.fuelCutProt = true; }
    const limitNm = read1(cal.torque.limitByRpm, s.rpm);
    if (crankNm > limitNm) { prot.push('torque'); e.torqueRetard = Math.min(30, e.torqueRetard + 30 * dt); } else if (e.leanT === 0) e.torqueRetard = Math.max(0, e.torqueRetard - 15 * dt);
    if (pedal < 3 && e.boostCutPsi > 0 && e.boostCutPsi < 99) e.boostCutPsi = 0;
  ...
  // ---- ROTATIONAL DYNAMICS.
  const starterNm = s.cranking ? STARTER_STALL_NM * clamp(1 - s.rpm / STARTER_FREE_RPM, 0, 1) : 0;
  const crankDrag = s.cranking
    ? ((derived.displacementL / 1000) * (1.3e5 + (pt ? (BARO_KPA - pt.map) * 1000 : 0))) / (4 * Math.PI)
    : 0;
  const acNm = s.running && aux.ac ? AC_NM : 0;
  const netNm = crankNm + starterNm - crankDrag - altNm - acNm - (s.running ? input.load : 0);
  s.omega = Math.max(0, s.omega + (netNm / INERTIA) * dt);
  ...
  const coolCap = faults.fan === 'failed' ? 135 : cal.protect.fanOnC;
  if (s.running) {
    ...
    s.oilC = Math.min(115, s.oilC + heatIn * dt * 0.7);
  } else {
  ...
  if (closedLoop) {
    ...
    s.ltft = clamp(s.ltft + s.stft * cal.fuel.ltftRate * (dt / 0.05), -cal.fuel.ltftLimit, cal.fuel.ltftLimit);
  } else if (s.running) {
  ...
  const noisy = (prev, v, l, n) => prev + (v - prev) * l + (Math.random() - 0.5) * 2 * n;
  const lopeAmp = s.running && s.rpm < 1500 ? ((derived.overlapDeg || 0) + e.camIn + e.camEx) * 0.9 : 0;
  s.lope = lopeAmp;
  s.sensedRpm = s.rpm < 30 ? 0 : noisy(s.sensedRpm, s.rpm, lag, 14 + lopeAmp);
  s.sensedMaf = noisy(s.sensedMaf, pt ? pt.maf : 0, lag * 0.8, 1.4);
  s.sensedMap = e.sMap;
  s.sensedIat = e.sIat;
