...
/** Ambient oil-pump characteristics: kPa per 1000 RPM hot, and relief pressure. */
const OIL_KPA_PER_KRPM = 95;
const OIL_RELIEF_KPA = 520;
...
export function injectorDeadTimeMs(volts, dpKpa = INJECTOR_RATED_DP_KPA) {
  const v = Math.max(6, volts);
  return Math.pow(13.5 / v, 1.6) * (1 + 0.0005 * (dpKpa - INJECTOR_RATED_DP_KPA));
}
...
export function fuelRail({ fuelSystem, mapKpa, baroKpa = BARO_KPA, demandLph, volts = 13.5, pumpHealth = 1 }) {
  ...
  const pumpLph = pump.lph * clamp(1 - (railGauge - INJECTOR_RATED_DP_KPA) / 900, 0.15, 1.3)
    * clamp(volts / 13.5, 0.3, 1.15) * pumpHealth;
...
export function coilOutput(dwellMs, volts, coil = COIL_OPTS[0]) {
  ...
  const R = coil.resistanceOhm + 0.9; // primary plus driver and harness
  ...
  return { kv: coil.kvMax * Math.sqrt(clamp(mj / coil.mjFull, 0, 1.2)), mj, ampsPeak: i };
}
...
export function oilPressureKpa(rpm, oilC, pumpHealth = 1) {
  if (rpm < 50) return 0;
  const viscosity = clamp(Math.exp(-(oilC - 90) / 38), 0.7, 6);
  return Math.min(OIL_RELIEF_KPA, (rpm / 1000) * OIL_KPA_PER_KRPM * Math.pow(viscosity, 0.55) * pumpHealth + 15);
}
