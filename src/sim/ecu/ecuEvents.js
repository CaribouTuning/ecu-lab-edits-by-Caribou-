...
export function ecuSweepEvents(points, { cal, hw, hardCut, endRpm }) {
  ...
  groupRuns(points, (p) => has(p, 'lean')).forEach((run) => {
    ...
    const misread = Math.abs(peak.sensedLambda - (peak.lambdaExhaust ?? peak.lambda)) > 0.03;
    events.push({
  ...
  groupRuns(points, (p) => has(p, 'fuelpressure') || p.fuelStarved).forEach((run) => {
    ...
    events.push({
      ...
      fix: peak.fuelStarved ? 'Fit a bigger fuel pump on BUILD → FUEL.' : 'Raise base fuel pressure, fit a return-style regulator, or turn on pressure compensation on TUNE → INJECTORS.',
    });
  });

  groupRuns(points, (p) => (p.knockFalse ?? 0) > 0.5).forEach((run) => {
  ...
  groupRuns(points, (p) => (p.knockUnheard ?? 0) > 0.3).forEach((run) => {
  ...
  groupRuns(points, (p) => (p.misfire ?? 0) > 5 && !(p.cutPct > 0)).forEach((run) => {
  ...
  const stoichRun = points.filter((p) => p.ecuStoich && Math.abs(p.ecuStoich / hw.fuel.stoich - 1) > 0.03);
  if (stoichRun.length) {
    const p = stoichRun[0];
    events.push({
      ...
      fix: 'On TUNE → FUEL, set the fuel the ECU assumes to match the tank, or fit an ethanol sensor and enable flex.',
    });
  }

  const wbMis = (hw.sensorHw?.wideband ?? 'lambda-0.5-1.5') !== cal.sensors.wideband
    && points.find((p) => p.openLoop === false && Math.abs(p.sensedLambda - (p.lambdaExhaust ?? p.lambda)) > 0.03);
  if (wbMis) {
    events.push({
