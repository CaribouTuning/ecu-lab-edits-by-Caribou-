...
import { BARO_KPA, DRIVETRAIN_EFF, KELVIN_OFFSET } from '../constants.js';
...
import { knockDetection } from './knockSensor.js';
...
export function cylinderDistribution(derived, configuration) {
  ...
  return out.map((c) => ({
    air: 1 + 0.035 * (c.x * c.x - meanSq) - 0.012 * c.x,
    chamberK: 7 * ((1 - c.x * c.x) - (1 - meanSq)),
    bank: c.bank,
  }));
}
...
export function resolveEcuPoint({
  ...
  // ---- KNOCK CONTROL.
  const peakEst = 0.62 * mapKpa;
  const det = knockDetection({
    rpm, peakBar: peakEst, thresholdV: read1(cal.ignition.knockThreshold, rpm),
...
export function ecuSteadyPoint({ cal, hw, cond, tables, rpm, loadKpa }) {
  ...
  if (cal.limiter.throttleCutRpm > 0 && rpm > hardCut - cal.limiter.throttleCutRpm) {
    protect.add('limiter');
    mod.loadScale = clamp(1 - (rpm - (hardCut - cal.limiter.throttleCutRpm)) / cal.limiter.throttleCutRpm, 0.15, 1);
  }
  if (soft > 0) {
    protect.add('limiter');
    if (cal.limiter.mode === 'retard') mod.extraRetardDeg += 15 * soft;
  ...
  for (let iter = 0; iter < 4; iter += 1) {
    ...
    if (hw.turboOn && cal.boost.overboostEnabled && pt.boostPsi > solved.target + cal.boost.overboostMarginPsi) {
      ...
      if (cal.boost.overboostAction === 'fuel-cut') { if (mod.cutFrac < 1) { mod.cutFrac = 1; changed = true; } } else if (mod.boostCutPsi < 99) { mod.boostCutPsi = 99; changed = true; }
    }
    ...
    if (P.egtEnabled && pt.egt > P.egtLimitC) {
      ...
      if (want > mod.extraFuelPct + 0.25) { mod.extraFuelPct = want; changed = true; protect.add('egt'); }
    }
    ...
    if (P.knockEnabled && hw.turboOn && pt.knockPull > P.knockRetardDeg && mod.boostCutPsi < 30) {
      mod.boostCutPsi += P.knockBoostCutPsi; changed = true; protect.add('knock');
    }
    if (P.dutyEnabled && hw.turboOn && pt.duty > P.dutyLimitPct && mod.boostCutPsi < 30) {
    ...
    if (P.fuelPressEnabled && resolved.sensed.rail.deltaKpa < P.fuelMinDeltaKpa && mod.cutFrac < 1) {
      ...
      if (hw.turboOn && mod.boostCutPsi < 99) { mod.boostCutPsi = 99; changed = true; }
    }
    if (P.sensorLimp && resolved.sensed.fault && hw.turboOn && mod.boostCutPsi < 99) {
      protect.add('limp'); mod.boostCutPsi = 99; changed = true;
    }
...
function applyAction(action, mod, cutPsi, turboOn) {
  ...
  if (action === 'torque') { if (mod.loadScale <= 0.5) return false; mod.loadScale *= 0.85; return true; }
  if (!turboOn || mod.boostCutPsi >= 30) return false;
...
export function solveOperatingPoint({ cal, hw, cond, rpm, loadKpa, throttlePct, gear, boostCutPsi = 0 }) {
  ...
  const iatGuessC = chargeTempK(hw.turboOn ? Math.max(...hw.boostCurve) * 0.8 : 0, hw.mods.intercooler, env) - KELVIN_OFFSET;
  const bt = hw.turboOn
  ...
  let cam = camTargets({ cal: cal.vvt, vvt: hw.vvt, rpm, mapKpa: Math.min(loadKpa, BARO_KPA) + target * 6.895 });
  let man = null;
...
function limitTorque({ cal, hw, cond, tables, rpm, loadKpa, throttlePct, gear, mod, limitNm }) {
  ...
  let lo = method === 'throttle' ? 0.15 : mod[knob];
