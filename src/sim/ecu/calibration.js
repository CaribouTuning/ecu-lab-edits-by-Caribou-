...
import { gateHoldPsi, dutyForHold } from './boost.js';
...
/**
 * The factory base-duty table recomputed for a wastegate, for when the actuator changes.
 * @param {{type: string, springPsi: number}} gate
 */
export function baseDutyFor(gate) {
  return map2(RPM, BOOST_AXIS, (_, psi) => dutyForHold(gate, psi).duty);
}

/**
 * Boost a pneumatic gate holds at zero duty, which open loop cannot go below.
 * @param {{type: string, springPsi: number}} gate
 */
export function springFloorPsi(gate) {
  return gate?.type === 'pneumatic' ? gateHoldPsi(gate, 0) : 0;
}

/**
 * A calibration brought up to the current shape: anything missing from an older saved
 * calibration comes from the default, and per-cylinder arrays follow the cylinder count.
 *
 * @param {object|null|undefined} cal
 * @param {Parameters<typeof defaultEcuCalibration>[0]} [ctx]
 * @returns {EcuCalibration}
 */
export function normaliseEcu(cal, ctx) {
  const def = defaultEcuCalibration(ctx);
  if (!cal) return def;
  const out = /** @type {any} */ ({ ...def });
  for (const k of Object.keys(def)) {
    if (k === 'version') continue;
    out[k] = typeof def[k] === 'object' && !Array.isArray(def[k])
      ? { ...def[k], ...(cal[k] ?? {}) } : (cal[k] ?? def[k]);
  }
  const cyl = ctx?.derived?.cyl ?? out.fuel.cylTrim.length;
  const fit = (a) => Array.from({ length: cyl }, (_, i) => a?.[i] ?? 0);
  out.fuel = { ...out.fuel, cylTrim: fit(out.fuel.cylTrim) };
  out.ignition = { ...out.ignition, cylTrim: fit(out.ignition.cylTrim) };
  return out;
}
...
export const ECU_META = [
  ...
  { path: 'config.stoichMode', section: 'fuel', group: 'Fuel type', label: 'Fuel the ECU assumes', kind: 'enum',
    ...
    help: 'Fuel mass comes from air mass and the stoichiometric ratio the ECU believes. Tell it gasoline with E85 in the tank and every commanded lambda is a third lean.' },
  ...
  { path: 'fuel.cylTrim', section: 'fuel', group: 'Per cylinder', label: 'Cylinder fuel trim', kind: 'cyl', unit: '%', min: -20, max: 20, step: 0.5, decimals: 1,
    help: 'Fuel added or removed on one cylinder. Only meaningful with cylinder modelling on (AIR tab): intake manifolds do not share air evenly, and a trim is how each cylinder is brought to the same mixture.' },
  ...
  { path: 'ignition.knockEnabled', section: 'spark', group: 'Knock control', label: 'Knock control', kind: 'bool', options: onOff,
    help: 'Off, the ECU never retards for knock — whatever the timing table says is what the engine gets.' },
  ...
  { path: 'sensors.map', section: 'sensors', group: 'Scaling', label: 'MAP sensor scaling', kind: 'enum',
    options: [{ id: 'auto', label: 'Matches fitted sensor' }, { id: '1bar', label: '1 bar' }, { id: '2.5bar', label: '2.5 bar' }, { id: '3bar', label: '3 bar' }, { id: '4bar', label: '4 bar' }],
  ...
  { path: 'sensors.wideband', section: 'sensors', group: 'Scaling', label: 'Wideband scaling', kind: 'enum',
    options: [{ id: 'lambda-0.5-1.5', label: 'λ 0.50–1.50 (0–5 V)' }, { id: 'afr-10-20', label: 'AFR 10–20 (0–5 V)' }, { id: 'afr-7.3-22.4', label: 'AFR 7.3–22.4 (0.5–4.5 V)' }],
