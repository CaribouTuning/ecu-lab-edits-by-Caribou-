...
/**
 * Deep copy of a table, so an edit never writes through to a shared default.
 * @template {EcuTable} T
 * @param {T} t
 * @returns {T}
 */
export function cloneTable(t) {
  ...
  return Array.isArray(a.y)
    ? /** @type {T} */ (/** @type {any} */ ({ x: [...a.x], y: [...a.y], z: a.z.map((r) => [...r]) }))
    : /** @type {T} */ (/** @type {any} */ ({ x: [...a.x], z: [...a.z] }));
}
...
/**
 * Whether a table has two axes.
 * @param {EcuTable} t
 * @returns {t is Map2D}
 */
export function is2D(t) {
  return Array.isArray(/** @type {any} */ (t)?.y);
}
