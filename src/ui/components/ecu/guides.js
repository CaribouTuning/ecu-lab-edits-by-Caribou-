...
export const PAGE_GUIDES = {
  fuel: {
    ...
    leave: 'On factory settings the engine gets exactly what your FUEL table asks for.',
  },
  injectors: {
  ...
  spark: {
    ...
    leave: 'Every correction starts at zero, so the engine gets exactly your SPARK table.',
  },
  airflow: {
  ...
  boost: {
    ...
    leave: 'Closed loop with the factory duty table reaches the BUILD boost curve by itself.',
  },
  vvt: {
...
export const FIELD_TIPS = {
  // FUEL
  'config.stoichMode': 'Which fuel the ECU assumes is in the tank. Leave on "Matches tank" unless you are recreating a mistake on purpose.',
  ...
  'fuel.comp': 'A final percentage on the fuel, by RPM and load. 100 = no change. For the last few percent once the AIR table is right.',
  ...
  'injector.maxDutyPct': 'The longest the injectors may stay open. Past about 90% they stop metering accurately.',
  // SPARK
  ...
  // AIRFLOW
  'config.airModel': 'How the ECU works out air: from pressure and the AIR table, from the airflow sensor, or both. Leave on the default.',
  ...
  'airflow.camVeCorr': 'Corrects the AIR table as the cam phaser moves. Needs cam phasers.',
