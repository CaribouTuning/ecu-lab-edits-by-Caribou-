...
import {
  ECU_META, defaultEcuCalibration, deriveEngine, ecuHardwareOf, getCal, setCal,
} from '../../../sim/index.js';
...
export function EcuSection({ section, title, icon, intro, liveVars = null, embedded = false }) {
  ...
  const sectionModified = ECU_META.some((m) => m.section === section
    && JSON.stringify(getCal(cal, m.path)) !== JSON.stringify(getCal(def, m.path)));
  ...
  if (meta.kind === 'bool') {
    return (
      <div className={styles.field}>
        <Toggle label={meta.label} sub={meta.help} checked={!!value} onChange={onChange} />
      </div>
    );
  }
  if (meta.kind === 'enum') {
