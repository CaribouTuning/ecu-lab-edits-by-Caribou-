...
import {
  ECU_META, defaultEcuCalibration, deriveEngine, ecuHardwareOf, getCal, sensorMismatches, setCal,
} from '../../../sim/index.js';
...
export function EcuSection({ section, title, icon, intro, liveVars = null, embedded = false }) {
  ...
  const mismatches = section === 'sensors' ? sensorMismatches(ecuHardwareOf(build).sensorHw, cal) : [];
    ...
    <section className={embedded ? styles.embedded : styles.section} aria-label={title}>
      ...
      {mismatches.length > 0 && (
        <div className={styles.mismatch} role="alert">
          ...
          {mismatches.map((m) => (
            <div key={m.path} className={styles.mismatchRow}>
              <div className={styles.mismatchText}>
                <b>{m.what}</b>: fitted {m.fitted}, but the ECU is set to {m.told}.
              </div>
              <button type="button" className={styles.fix} onClick={() => set(m.path, m.value)}>Match BUILD</button>
            </div>
          ))}
        </div>
      )}
