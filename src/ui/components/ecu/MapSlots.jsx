      ...
      <div className={styles.row} role="group" aria-label="Map slot">
        ...
        {maps.map((m, i) => (
          <button
            ...
            title={i === active ? 'Running' : m ? 'Stored calibration' : 'Empty — takes a copy of the running map'}
            onClick={() => dispatch({ type: ACTIONS.SWITCH_MAP, index: i })}
          >
            {i + 1}
            {i !== active && !m && <span className={styles.empty} aria-hidden="true">·</span>}
          </button>
        ))}
      </div>
      {!compact && (
        <div className={styles.utils}>
          <select className={styles.select} value={copyTo} aria-label="Copy the running map to slot"
            ...
            {others.map((i) => <option key={i} value={i}>Map {i + 1}</option>)}
          </select>
