...
export function FuelSystemScreen({ active, onToggle }) {
  ...
  return (
    ...
    >
      ...
      <Seg label="Wideband controller" options={Object.entries(LINEAR_SCALES.wideband).map(([id, s]) => ({ id, label: s.label }))} value={sensorHw.wideband} onChange={(v) => setSensors({ wideband: v })} equal />
      <div className={styles.hint}>The ECU's scaling for it lives on TUNE › SENSORS, and has to match.</div>
    </BuildSection>
  );
}
