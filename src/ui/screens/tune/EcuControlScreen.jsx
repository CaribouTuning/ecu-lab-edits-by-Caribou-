...
/** What each view says about itself, above its settings. */
const INTROS = {
  boost: 'The ECU decides how much boost it wants and drives the wastegate to get it; the turbo then makes whatever its power balance allows under that ceiling. The BUILD boost curve is the base target everything here starts from.',
  vvt: 'Cam phasers rotate a whole camshaft against the crank. Advancing the intake closes its valve earlier — more charge trapped at low speed, less at high — and every phaser move changes overlap, and with it how much burned gas stays behind.',
  idle: 'At idle the driver has let go and the ECU holds the engine up on its own: air for the slow drift, spark for the fast corrections, feed-forward for loads it knows are coming.',
  protect: 'What the ECU does when something is going wrong. Every protection acts on what the ECU can measure — which is why a sensor calibrated wrong can fool one.',
  torque: 'Torque management caps what the engine may deliver, by gear and by engine speed, and traction control takes torque out when the tyre spins. How the torque is removed matters as much as how much: each actuator has its own speed and its own side effects.',
};
...
export function EcuControlScreen({ section, title, icon, liveVars }) {
  ...
  return (
    <div className={styles.wrap}>
      ...
      <EcuSection section={section} title={title} icon={icon} intro={INTROS[section]} liveVars={liveVars} />
    </div>
  );
}
