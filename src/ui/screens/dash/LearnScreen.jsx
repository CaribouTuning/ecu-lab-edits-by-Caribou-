  ...
  return (
    ...
    >
      ...
      <ExpandableInfo title="29. Why the engine sounds the way it does">
        The sound here is not a recording. It is generated from the same numbers the physics
        already computes, so every change you make is audible for a real reason.
        <br /><br /><b className={styles.em}>Gas leaves a cylinder two ways.</b> When the exhaust valve cracks, the cylinder may still be above atmospheric, and that difference drives a sharp pulse out through a small hole — <b className={styles.em}>blowdown</b>. The cylinder empties on an exponential whose time constant is a volume divided by a flow:
        <br /><span className={styles.formula}>tau = V_cylinder / (c × A_valve × Cd)</span>
        <br />A big cylinder behind a small valve empties slowly, so its pulse is long and low. A small one cracks. That is why a big-bore loafer thumps and a small four barks, and it is not a setting.
        <br /><br />Then the piston sweeps the rest out — <b className={styles.em}>displacement</b>. This matters more than it sounds: on a closed throttle with the fuel cut — the overrun, or the rev limiter — the cylinder reaches valve opening at around 48 kPa, <i>below</i> atmospheric, so there is no blowdown at all. An engine modelled on blowdown alone goes silent there. What you hear on the overrun is the piston pushing. A firing idle is different: it still burns, and reaches the valve at about one and a half atmospheres, which is the soft putt of each event.
        <br /><br /><b className={styles.em}>Most of what you hear is turbulence.</b> Gas does not leave a cylinder as a click. It leaves over the whole exhaust stroke, and the rushing flow is broadband noise whose loudness follows the gas speed through the stroke. Faster gas sheds smaller eddies, which sound higher — so the noise is not a hiss but a band centred on a frequency set by how fast the gas is moving, and it climbs as you rev. An early version of this model left the turbulence out and made each event a clean pulse rung through pipes. Measured against real recordings it was twenty to a hundred times too tonal, and sounded like a computer going boop. Events also overlap: a V8 fires every 90° and each exhaust stroke lasts 180°, so two are always in flight — which is why a V8 burbles continuously where a four putts.
        <br /><br /><b className={styles.em}>The pipes make the pitch.</b> A pulse runs down the primary, reaches the collector and reflects. Crucially, <b className={styles.em}>an open end reflects it inverted</b> — it comes back as a rarefaction, a vacuum wave. Burns Stainless describes header tuning as timing exactly that negative wave to arrive during valve overlap, where it helps scavenge the cylinder. That inversion is also what sets the note:
        <br /><span className={styles.formula}>primary (closed at the valve, open to the collector): odd multiples of c / 4L</span>
        <br /><span className={styles.formula}>collector + tail (open both ends): every multiple of c / 2L</span>
        <br />A quarter-wave resonator missing its even harmonics sounds completely different from a tube that rings them all — that is the hard edge headers give an engine.
        <br /><br /><b className={styles.em}>And c is not 343 m/s.</b> The speed of sound goes as the square root of absolute temperature, and exhaust gas is hot: Burns quotes about 1,700 ft/sec, near 520 m/s. Tune the pipes for cold air and the whole note sits an octave out. It also means <b className={styles.em}>retarding the spark raises the pitch</b>: a later burn is still going when the valve opens, the gas is hotter, c rises, and every tube in the system tunes up. Nothing in the audio code knows what ignition timing is — it arrives as a temperature.
        <br /><br /><b className={styles.em}>Where the pulses fall is the layout.</b> One pulse per firing event, at the crank angles that engine actually fires at:
        <br /><span className={styles.formula}>firing Hz = RPM ÷ 60 × cylinders ÷ 2</span>
        <br />A cross-plane V8's banks are offset, so within one bank the gaps alternate long and short — and that unevenness <i>is</i> the rumble. Even-fire the same engine and it stops sounding like a V8. A 60/120° V6 fires evenly and rings hard and hornlike. A four fires twice a revolution, far enough apart to hear separately, which is the hollow four-cylinder sound.
        <br /><br /><b className={styles.em}>No two cycles are the same.</b> Combustion varies from one cycle to the next — about 1-3% under load and far more at idle — and no event arrives exactly on time. A perfectly regular train of pulses has a perfectly regular spectrum, and the ear hears that as synthesis instantly. Each event here is scattered in size and timing by about what a real engine does.
        <br /><br /><b className={styles.em}>What else you can hear.</b> A bigger pipe reflects less at the mouth and rings less, which is why a straight-through system sounds opened up rather than boomy. A muffler absorbs the reflection instead of returning it, and takes the top off the turbulence: a straight-through system passes far more of it, which is most of why a race car sounds raspy and a road car does not. A turbine in the stream takes energy out of the gas, which is why turbo cars are quieter. More boost or load raises the pressure at valve opening and the pulse hits harder. Overlap makes some cycles burn weakly, and that uneven firing at low speed is lope.
      </ExpandableInfo>

      <ExpandableInfo title="30. Sources for the sound model">
        The acoustics above is standard published material, not invented, and where a source disagreed with this app the app was changed.
        <br /><br /><b className={styles.em}>Burns Stainless, "Exhaust Header Theory".</b> The load-bearing source. It gives the pressure-wave speed in exhaust gas as around 1,700 ft/sec against a gas particle velocity over 350 ft/sec, and states plainly that at an open end "part of the wave is reflected back towards the cylinder as a negative pressure (or vacuum) wave". <b className={styles.em}>Checking it caught a real error here:</b> this app reflected the primary in phase, like a closed pipe, so it rang every harmonic instead of the odd ones a quarter-wave resonator rings. It also explains why collector design matters — a 4-into-1 presents a larger area change than a 2-into-1 and so returns a stronger wave.
        <br /><br /><b className={styles.em}>Flatirons, "Resonance and Reverberation".</b> The plain-language version of the same ideas: resonance when tube length matches the wavelength, constructive and destructive interference setting tone, and larger diameter giving a louder, more open note while smaller pipe muffles it.
        <br /><br /><b className={styles.em}>HowStuffWorks, "How Mufflers Work".</b> Why a muffler is not simply a restriction: resonator chambers are tuned to cancel particular frequencies by destructive interference, while packing absorbs broadband energy. Modelled here as absorption at the reflection rather than a volume control.
        <br /><br /><b className={styles.em}>Car and Driver, "Why Various Engine Types Sound So Different".</b> On firing intervals and crank arrangement as the origin of layout character — the cross-plane V8 rumble in particular.
        <br /><br /><b className={styles.em}>PhET, "Sound Waves"</b> (University of Colorado Boulder), and <b className={styles.em}>Britannica, "Sound (physics)"</b>. The fundamentals underneath all of it: pressure waves, wavelength and frequency, interference, standing waves and resonance. Start here if the rest of this article assumed too much.
        <br /><br /><b className={styles.em}>ScienceDirect, engine noise and vibration.</b> Reference material on exhaust system acoustics and transmission loss, for the level above this app.
        <br /><br />One number in the renderer is deliberately <i>not</i> physical. A real engine spans roughly 75 dB(A) at idle to 105 at full load, and rendered linearly that leaves idle inaudible on a phone. The physics is computed as physics and then raised to a power below one at the very last stage, which keeps every response the model produces and compresses the range to something you can hear at both ends. It is the one perceptual number, and it is marked as such in the code.
      </ExpandableInfo>
    </BuildSection>
  );
}
