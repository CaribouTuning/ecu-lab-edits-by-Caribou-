 ...
 * the output ceiling and hard-clipped, a smooth saturation stage keeps the harmonics that
 * clipping produced and caps the output below full scale, so nothing reaches the speaker
 * hard-clipped however hard the engine is pushed.
 *
 ...
 * (a crest factor of 0.5-6 dB). At 1.5 the saturation still puts back the harmonics that
 * pinning gave a V8 and a four, with 5-9 dB of crest left, so the pulses stay pulses.
 */
...
export function createEngineAudio(ctx) {
  ...
  // into the ceiling, and the hard clip there was part of its sound: it turned the low sine
  // thump in every pulse into a buzz rich in upper harmonics, which is most of what made a
  // V8 or a four big and audible on a phone. Take it away and those two went quiet and
  // synthetic (see `thumpBalance` in pulseExhaust.js for the rest of that fix). So the
  // harmonics stay and the hard edge goes: the same stage, as a smooth tanh curve instead of
  // a wall, and then a fixed output level that no input can push past.
