 ...
 *
 * The note itself is not built here at all. It comes out of `pulseExhaust.js`, a port of
 * the reference build's (v4.8) exhaust: one combustion pulse per firing event through a
 * resonant pipe, a looped cycle at high revs, and a faint oscillator and combustion-noise
 * underlay. This file sends it the build when that changes and the engine's state a few
 * times a second.
 *
 ...
 * drive.
 *
 * THE OUTPUT STAGE IS THE REFERENCE'S TOO. Exhaust pulses are sharp transients, so raw
 * gain clips long before it sounds loud. A fast limiter lets the running level come up a
 * long way while the peaks stay clean — the same reason real engine recordings are
 * compressed before anyone hears them — and a soft brickwall after the make-up gain
 * catches whatever gets past it, so nothing reaches the output hard-clipped.
 *
...
/**
 * Make-up gain after the limiter, scaled by the player's volume.
 *
 * The reference used 2.4, but a browser's DynamicsCompressor already applies make-up gain
 * of its own, so 2.4 on top of it put the output about 8 dB over full scale. This lands a
 * full-throttle V8 with its peaks near -2 dBFS and no clipped samples, measured end to end.
 */
const MAKEUP_GAIN = 0.6;

/**
 * Level of the note going into the limiter.
 *
 * The note's internal balance is v4.8's exactly; this and the make-up gain below are the
 * two output numbers that are not, and deliberately. Rendered end to end, the reference's
 * staging sat pinned at full scale — 95% of a full-throttle V8's samples clipped — which
 * is the "distorted, capped out" sound. The input is still hot enough for the limiter to
 * do the levelling it was there for, so idle and full load sit close together the way
 * they did in the reference; it is the make-up after it that brings the whole down.
 */
const NOTE_TRIM = 0.4;
...
export function updateEngineAudio(a, frame) {
  ...
  // and each one is a scheduled automation event. The pulse scheduler works 140 ms ahead
  // of the clock, so updating it at this rate costs it nothing.
  if (t - a.paramsAt < 1 / PARAM_HZ) return;
  a.paramsAt = t;

  // RASP. Burning later dumps more of the heat out through the valve instead of into the
  // crank, which is why a retarded engine sounds harder and raspier.
  ...
  if (a.exhaust) {
    ...
    schedulePulseExhaust(a.exhaust, a.ctx, {
      rpm,
      level: audible ? (cut ? 0.14 : 0.42 + load * 0.62) : 0,
      load,
      overlapDeg: drive.overlapDeg,
      audible,
    });
    tonePulseExhaust(a.exhaust, a.ctx, {
      rpm, load, rasp, richness, knock: drive.knockLevel, cranking, catBack, audible,
      overlapDeg: drive.overlapDeg,
    });
  }
  a.exhaustGain.gain.setTargetAtTime(audible ? NOTE_TRIM : 0, t, 0.08);
