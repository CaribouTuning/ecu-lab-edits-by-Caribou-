 ...
 * tested and signed off. For a presentation layer
 * the ear is the acceptance test, so the note is built the way the reference built it, with
 * the reference's own numbers. Where a number here differs from v4.8 the comment says why.
 *
 ...
 * @returns {AudioBuffer}
 */
function renderPulse(ctx, variant) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    data[i] = (Math.sin(2 * Math.PI * thumpHz * x) * 0.9 + (Math.random() * 2 - 1) * crack) * env;
  }
  return buffer;
}
...
function renderCycle(ctx, events, variant) {
  ...
  for (const event of events) {
    ...
    for (let i = 0; i < eventLength; i++) {
      ...
      data[j] += (Math.sin(2 * Math.PI * thumpHz * x) * 0.9 + (rnd() * 2 - 1) * crack) * env;
    }
  }
  return buffer;
}
...
export function tonePulseExhaust(a, ctx, frame) {
  ...
  // A four is rough and buzzy, a V8 leans on its low-order rumble, a six sits between.
  a.oscB.detune.setTargetAtTime(isFour ? 16 : isEight ? 6 : 9, t, 0.2);
  a.oscGain.gain.setTargetAtTime(isFour ? 0.045 : isEight ? 0.025 : 0.035, t, 0.1);
  a.subGain.gain.setTargetAtTime(
    (isFour ? 0.05 : isEight ? 0.16 : 0.08) + contMix * (isEight ? 0.10 : 0.05), t, 0.1);

  a.body.frequency.setTargetAtTime(safe((isEight ? 240 : isFour ? 420 : 320) / dispDepth, 60, 2000), t, 0.15);
  a.tone.frequency.setTargetAtTime(
    safe((300 + fire * 7 + load * 2400) * diaOpen / dispDepth * (1 + rasp * 0.45 + crBite), 80, 18000), t, 0.05);
  // Each layout its own resonance: a V8's is low, broad and loose so the uneven pulses
  // blur into a rumble; a V6's mid and ringing, hence the horn; a four's high and tight.
  a.tone.Q.setTargetAtTime((isFour ? 4.5 : isEight ? 1.1 : 6.0) * (1 + crBite), t, 0.1);
  a.body.Q.setTargetAtTime(isEight ? 0.6 : isFour ? 1.4 : 3.6, t, 0.15);
  a.body2.Q.setTargetAtTime(isEight ? 0.9 : isFour ? 2.2 : 5.0, t, 0.15);
  a.body2Gain.gain.setTargetAtTime(
    ((isEight ? 0.10 : isFour ? 0.20 : 0.34) + rasp * 0.18) * (0.35 + 0.65 * load), t, 0.12);
  a.busTrim.gain.setTargetAtTime(isEight ? 1.25 : isFour ? 0.95 : 1.05, t, 0.15);
  ...
  a.pipeOut.gain.setTargetAtTime(
    frame.audible ? (isEight ? 1.10 : isFour ? 0.68 : 0.88) * (0.45 + 0.55 * flow) : 0, t, 0.12);
