 ...
 * whether or not there is any overpressure left, which matters more than it sounds:
 * AT IDLE THERE IS NO BLOWDOWN AT ALL. A throttled engine reaches valve opening at around
 * 48 kPa, well BELOW atmospheric, so a model with only a blowdown term goes silent exactly
 * where a real engine does not — and then slams back in under load, which is what a first
 * cut of this did. An idling engine is quiet, not mute, and what you hear is this term.
 *
...
export function blowdown(geometry, evoKpa, gasTempK, rpm = 800) {
  ...
  const amplitude = safe(
    BLOWDOWN_GAIN * Math.max(0, pressureRatio - 1) + DISPLACEMENT_GAIN * displacementMach,
    0.01, 4);
  return { tau, jet, pressureRatio, displacementMach, amplitude };
}
...
/**
 * Build the looped cycle for the current geometry.
 *
 * SEPARATE FROM THE EXCITATION ON PURPOSE. These are always-running sources with a
 * smoothed gain, so replacing them costs a fade from zero — and an earlier version
 * rebuilt them whenever the blowdown moved between two rendered time constants. Gas
 * temperature moves with load, so at high RPM that thrashed every frame: each new gain
 * node started at zero and got 50 ms of a 180 ms ramp before being replaced, and the whole
 * loop layer sat silent. Two and a half seconds of exact zero in the middle of a pull.
 *
 * The loops are a function of the FIRING PATTERN, which only changes when the engine does.
 * Where the pulse shape has moved since, playback rate carries it — the same resampling
 * the scheduled pulses use.
 *
 * @param {Record<string, any>} a
 * @param {AudioContext} ctx
 */
function rebuildLoops(a, ctx) {
  ...
  for (let v = 0; v < LOOP_VARIANTS; v++) {
    ...
    src.buffer = renderCycle(ctx, a.events, a.pulseTau, a.jet, v);
    src.loop = true;
...
export function schedulePulseExhaust(a, ctx, frame) {
  ...
  const drive = safe(a.amplitude ?? 0.3, 0.01, 4);
