...
import {
  ...
  DEFAULT_BOOST, DEFAULT_ENGINE_CONFIG, DEFAULT_MODS, ENGINE_PRESETS, EXHAUST_DIA_OPTS,
...
export function EcuLabApp() {
  ...
  const {
    ...
    live, activeJob, completedJobs, jobResult,
  } = session;
  ...
  // `histogram`, `live`, `throttleInput` and `journeyStep` were all declared here as
  // local state on this branch. They are the store's now, and are read from `session`
  // above; a second copy of any of them would be a second source of truth.
  //
  // HOME opens on the jobs board rather than on the live engine, because LIVE is its
  // own tab on this branch and is no longer a section of this screen.
  const [dashSection, setDashSection] = useState('jobs');
  // Whether a click on the calibration grid extends the selection into a range. A grid
  // interaction mode, so it sits with `tuneView` rather than in the tune slice, which
  // holds what is selected rather than how selecting behaves.
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        {tab === 'dash' && (
          <div style={{ padding: 16 }}>
            {/* Step 2 is the LIVE step, and LIVE is its own tab on this branch rather
                than a section of HOME, so its banner moved with it. */}
            <BuildSection
            ...
            >
              ...
              {activeJob != null && (
                <Panel style={{ marginBottom: 12, borderColor: T.acc }}>
                  ...
                  <button onClick={() => { dispatch({ type: ACTIONS.SET_SESSION_FIELD, field: 'activeJob', value: null }); dispatch({ type: ACTIONS.SET_SESSION_FIELD, field: 'jobResult', value: null }); }} style={{
            ...
            >
              ...
              <div style={{ fontSize: 11, letterSpacing: 1, color: T.accInk, fontWeight: 800, margin: '14px 0 8px' }}>PART 4 · BEYOND THE SIMULATOR</div>
        ...
        {tab === 'live' && (
          <div style={{ padding: 16 }}>
            ...
            {/* The engine's own state machine, in a line. It was the collapsed section's
                subtitle on HOME; moving the panel onto its own tab must not lose it,
                because it is the only place the coolant temperature is readable at a
                glance and the only thing that distinguishes cranking from running
                before the gauges settle. */}
            <div style={{ fontSize: 11.5, color: T.ink2, fontWeight: 700, marginBottom: 8 }}>
              {live.running
                ? `Running · ${Math.round(live.sensedRpm)} RPM · ${Math.round(live.coolantC)}°C`
                : live.cranking ? 'Cranking…' : 'Off'}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 7 }}>
                  {/* START was filled with `ok`. Green here is decoration, not state —
                      the engine is not running when the button says START — and spending
                      a status colour on an action is the rule Toggle's docstring closed.
                      It takes the accent; STOP is the secondary state and takes `ghost`.
                      Not `danger`: shutting an engine down destroys nothing. */}
                  <Button
