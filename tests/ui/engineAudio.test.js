...
describe('what makes it sound real', () => {
  it('limits the way the reference build did, with a brickwall behind it', () => {
    ...
    // reach the output hard-clipped, which is heard as a tearing edge; the brickwall after
    // the make-up gain rounds anything that gets past the attack. Rendered end to end, a
    // V8 pull through this chain peaks below full scale with no clipped samples.
    const ctx = stubContext();
