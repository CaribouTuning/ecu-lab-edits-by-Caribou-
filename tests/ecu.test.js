...
describe('the default calibration is the original model', () => {
  ...
  it.each(S.ENGINE_PRESETS.map((p) => [p.id]))('pulls the %s preset to the same power and log', (id) => {
    ...
    expect(b.peakHp).toBe(a.peakHp);
    expect(b.peakTq).toBe(a.peakTq);
    // Within a pound-foot everywhere: the boost-by-throttle curve is a table, read with
    // linear interpolation, where the original model used the exact square.
