...
describe('what the engine is doing', () => {
  it('hits harder when there is more pressure behind the valve', () => {
    ...
    // map. A cylinder at four atmospheres pushes far harder than one at one and a half.
    const { ctx, a } = model();
    const quiet = [];
    const loud = [];
    const capture = (into) => {
      const g = ctx.createGain;
      ctx.createGain = (...args) => { const n = g(...args); into.push(n); return n; };
      return () => { ctx.createGain = g; };
    };
    let restore = capture(quiet);
    pulses(a, ctx, { evoKpa: 150 });
    restore();
    restore = capture(loud);
    pulses(a, ctx, { evoKpa: 600 });
    restore();
    const peak = (ns) => Math.max(...ns.map((n) => n.gain.value));
    expect(peak(loud)).toBeGreaterThan(peak(quiet) * 2);
  });
