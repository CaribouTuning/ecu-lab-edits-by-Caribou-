...
import { exhaustGeometry, firingEvents } from '../src/sim/acoustics.js';
import {
  createPulseExhaust, setPulseExhaustGeometry, schedulePulseExhaust, tonePulseExhaust,
  silencePulseExhaust, wakePulseExhaust, thumpBalance,
...
describe('the thump, balanced to the six', () => {
  const balance = (c) => thumpBalance(firingEvents(c), 70, 40, 0.04);

  it('leaves an even-firing six exactly as the reference voiced it', () => {
    expect(balance('V6')).toBeCloseTo(1, 6);
    expect(balance('I6')).toBeCloseTo(1, 6);
  });

  it('takes thump out of a V8 and a four, whose events do not cancel it', () => {
    // A V8's uneven spacing and a four's wide gaps let the low sine in every pulse sum
    // into a steady tone a phone cannot play: quiet and synthetic.
    expect(balance('V8')).toBeLessThan(0.6);
    expect(balance('I4')).toBeLessThan(0.7);
  });

  it('never adds thump', () => {
    for (const c of ['I4', 'I6', 'V6', 'V8']) expect(balance(c)).toBeLessThanOrEqual(1);
  });

  it('keeps the full thump at idle, where a V8\'s is its rumble, and balances it once fused', () => {
    const m = model(geom(), 'v8');
    expect(m.a.pulsesFused).not.toBe(m.a.pulses);
    const idle = run(m, { rpm: 700 });
